...
@allure.description('Тестовые данные для 2-х пользователей')
class Contents:
    ...
    ADD_COMMENT = 'Комментарий_Курьеруъ Comment_111'
...
@allure.title('Текст вопросов и ответов на главной странице для автотеста test_main_page.py')
@allure.description('Вопросы продублированы, так как было требования избегать хардкода. '
'Здесь тот же текст, но для asserta в параметризованном тесте, так как текст выше задейстстован в локаторах ...'
'... параметризованного теста. Как результат - в случае несоответствия ОР и ФР выводится не assertion error, а ... '
'... timeout exception, так как перед сравнением ОР и ФР метод ищет элемент с тем же самым текстом ...'
'... (см ANSWER_1, ANSWER_2) Итд. Единственный найденный способ вынести хардкод из самого теста - создать копии...'
'... переменных, которые нигде не задействованы. Если есть еще какой-то вариант, избегающий ... '
'... дублирование переменных в этом файле - будет исправлено. ')
